...
class Expan(object):
    ...
    def __init__(self, args, cls_names, model_name='bert-base-uncased'):
        ...
        # set the number of target entities per iteration if using improved
        # window search algorithm
        self.use_improved = False
        if args.use_improved:
            self.use_improved = True
            self.num_target_entities_per_iteration = args.num_target_entities_per_iteration
            if self.num_target_entities_per_iteration is None:
                self.num_target_entities_per_iteration = 3
    ...
    def expand(self, query_sets, target_size=103, ranking=True, mu=9,
        ...
        while cnt_iter < total_iter and flag_stop is False:
            ...
            if not self.use_improved:
                print('\n\nUsing original window search\n\n')
                expanded_sets = self.expand_(seed_sets, target_size, ranking, mu + cnt_iter * 2,
                                             init_win_size, win_grow_rate, win_grow_step)
            else:
                print('\n\nUsing improved window search\n\n')
                expanded_sets = self.expand_improved_(seed_sets, target_size, ranking, mu + cnt_iter * 2,
                                                      init_win_size, win_grow_rate, win_grow_step)
    ...
    def expand_(self, seed_sets, target_size, ranking, mu, init_win_size, win_grow_rate, win_grow_step):
        ...
        print(f'Original Window Search Time: {end - start}')
        
        if not self.use_improved:
            with open('original_search_time.txt', 'a') as file:
                    file.write(f'{end-start}\n')
        else:
            with open('improved_search_time.txt', 'a') as file:
                    file.write(f'{end-start}\n')
    ...
    def expand_improved_(self, seed_sets, target_size, ranking, mu, init_win_size, win_grow_rate, win_grow_step):
        expanded_sets = seed_sets
        ...
        while len(expanded_sets[0]) < target_size:
            ...
            for i, cls in enumerate(self.cls_names):
                ...
                ################################################################
                #          HERE WE IMPLEMENT THE IMPROVED WINDOW SEARCH
                ################################################################
                
                """ Improved Window Search """
                cnt = 0
                tgt_eid = []    # modified
                min_KL_div = float('inf')
                set_dist = np.zeros(self.len_vocab)
                ...
                for index in indexs:
                    ...
                    if eid in eid_out_of_sets:
                        ...
                        # if the length of the list of entities to add is less
                        # than the target length per iteration, add the entity
                        # and record its KL divergence
                        if len(tgt_eid) < self.num_target_entities_per_iteration:
                            
                            # add the eid to the list of target eids
                            tgt_eid.append({
                                'eid': eid, 
                                'KL_div': KL_div
                                })
                            # update the minimum KL divergence.. the minimum KL divergence
                            # required for another entity to be added to the list is the
                            # max KL divergence found in the list
                            min_KL_div = max(tgt_eid, key=lambda x: x['KL_div'])['KL_div']
                            continue
                        if KL_div < min_KL_div:
                            # remove the target entity with the highest KL_div
                            # in the list to make room for the new, more similar
                            # entity with lower KL_div
                            tgt_eid = sorted(tgt_eid, key=lambda x:x['KL_div'])
                            tgt_eid.pop()
                            
                            # add the new target id to the list and update the
                            # KL_divergence
                            tgt_eid.append({
                                'eid': eid,
                                'KL_div': KL_div
                ...
                for eid in tgt_eid:                                # modified
                    expanded_sets[i].append(eid['eid'])            # modified
                    eid_out_of_sets.remove(eid['eid'])             # modified
        ...
        print(f'Improved Window Search Time: {end - start}')
        
        if not self.use_improved:
            with open('original_search_time.txt', 'a') as file:
                    file.write(f'{end-start}\n')
        else:
            with open(f'improved_search_time_{str(self.num_target_entities_per_iteration).zfill(3)}_entities_per_it.txt', 'a') as file:
                    file.write(f'{end-start}\n')

...
if __name__ == '__main__':
    ...
    parser.add_argument('-scores_calculated', action='store_true', help='specify whether the scores have already been calculated and recorded')
    parser.add_argument('-use_improved', action='store_true', help='specify whether to use improved window search')
    parser.add_argument('-num_target_entities_per_iteration', type=int, default=3, action='store', help='specify how many target entities to add for each iteration in the window search algorithm')
    ...
    if args.ensemble:
        top_model_ids = []
        model_ids = np.array(range(1, int(args.num_model)+1))

    if args.pretrained_model is not None:
        if args.scores_calculated:
            scores = []
            for model_id in model_ids:
                score_txt_file_path = os.path.join(args.save_path + str(model_id),
                                                   'score.txt')
                scores.append(float(np.loadtxt(score_txt_file_path)))
                
            top_model_ids = model_ids[np.argsort(scores)[:int(args.num_top_model)]]
            print(top_model_ids)
            print("COMPLETED-------------------------------------------\n-\n-\n-")
        elif args.ensemble:
    ...
    if args.ensemble:
        ...
        if not os.path.exists('/home/nianyli/Desktop/code/information_retrieval/final_project/ProbExpan/data/wiki/entity2dist'):
    ...
    with open(os.path.join(args.dataset, args.output, 'summary.txt'), 'w') as file_summary:
        for i in range(0, num_query_per_class):
            ...
            print('MAP %.6f %.6f %.6f %.6f\n' %
                  ...
                  file=file_summary)
            print(f'MAP {sum(AP10s) / num_class} {sum(AP20s) / num_class} {sum(AP50s) / num_class} {sum(AP100s) / num_class}')
            if not expan.use_improved:
                with open('original_search_map.txt', 'a') as file:
                    file.write(f'MAP {sum(AP10s) / num_class} {sum(AP20s) / num_class} {sum(AP50s) / num_class} {sum(AP100s) / num_class}\n')
            else:
                with open(f'improved_search_map_{str(expan.num_target_entities_per_iteration).zfill(3)}_entities_per_it.txt', 'a') as file:
                    file.write(f'MAP {sum(AP10s) / num_class} {sum(AP20s) / num_class} {sum(AP50s) / num_class} {sum(AP100s) / num_class}\n')
        ...
        print(f'Total MAP {MAPs[0] / num_query_per_class} {MAPs[1] / num_query_per_class} {MAPs[2] / num_query_per_class} {MAPs[3] / num_query_per_class}')
        if not expan.use_improved:
            with open('original_search_map.txt', 'a') as file:
                    file.write(f'Total MAP {MAPs[0] / num_query_per_class} {MAPs[1] / num_query_per_class} {MAPs[2] / num_query_per_class} {MAPs[3] / num_query_per_class}\n')
        else:
            with open(f'improved_search_map_{str(expan.num_target_entities_per_iteration).zfill(3)}_entities_per_it.txt', 'a') as file:
                    file.write(f'Total MAP {MAPs[0] / num_query_per_class} {MAPs[1] / num_query_per_class} {MAPs[2] / num_query_per_class} {MAPs[3] / num_query_per_class}\n')
